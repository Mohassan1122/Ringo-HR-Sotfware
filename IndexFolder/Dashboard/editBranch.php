...
if (strlen($_SESSION['company_email']) == 0) {
    ...
    if (isset($_POST['submit1'])) {
        ...
        $result = mysqli_query($conn, "UPDATE branch set city='" . $city . "', address='" . $address . "' WHERE id='" . $eid . "'");

        if ($result) {
            echo "<script>alert('Branch Office updated successfully');</script>";
            echo "<script>window.location.href='./BranchSettings.php';</script>";
        } else {
                    ...
                    <div class="container mt-5">
                        <div class="row d-flex justify-content-center align-items-center">
                            <div class="col-md-8">
                                ...
                                <?php
                                $eid = intval($_GET['empid']);
                                $query = $conn->query("SELECT * FROM branch where id=$eid");
                                while ($result = $query->fetch_array()) :
                                ?>
                                    <h1 class="text-center">Add Branch</h1>
                                    <div class="tab">
                                        <h6>State/City</h6>
                                        <p> <input value="<?php echo htmlentities($result['city']); ?>" type="text" name="city"></p>
                                    </div>
                                    <div class="tab">
                                        <h6>Address</h6>
                                        <p><input value="<?php echo htmlentities($result['address']); ?>" type="text" name="address"></p>
                                    </div>
                                    <?php endwhile; ?>
                                    <div class="tab">
                                        <button type="submit" name="submit1" class="btn btn-primary">Add Location</button>
                                    </div>
                                </form>
                            </div>
                        </div>
