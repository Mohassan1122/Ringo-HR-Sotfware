...
if (strlen($_SESSION['company_email']) == 0) {
    ...
    if (isset($_POST['update2'])) {
        ...
        $result = mysqli_query($conn, "UPDATE tbrole set role='" . $role . "', dept='" . $dept . "' WHERE id='" . $eid . "'");

        if ($result) {
            echo "<script>alert('Role updated successfully');</script>";
            echo "<script>window.location.href='./roleSettings .php';</script>";
        } else {
                    ...
                    <div class="container mt-5">
                        <div class="row d-flex justify-content-center align-items-center">
                            <div class="col-md-8">
                                ...
                                <?php
                                $eid = intval($_GET['empid']);
                                $query = $conn->query("SELECT * FROM tbrole where id=$eid");
                                while ($result = $query->fetch_array()) :
                                ?>
                                    <h1 class="text-center">Add Role</h1>
                                    <div class="tab">
                                        <h6>Role Name</h6>
                                        <p> <input value="<?php echo htmlentities($result['role']); ?>" type="text" name="role"></p>
                                    </div>
                                    ...
                                    <?php endwhile; ?>
                                    <div class="tab">
                                        <button type="submit" name="update2" class="btn btn-primary">update Role</button>
                                    </div>
                                </form>
                            </div>
                        </div>
